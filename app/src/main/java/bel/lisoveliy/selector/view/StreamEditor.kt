...
import bel.lisoveliy.selector.viewmodels.StreamEditorVM.URLField
import bel.lisoveliy.selector.viewmodels.StreamEditorVM.TitleField
...
object StreamEditor {
    ...
    fun RadioSelector(){
        ...
        Row(verticalAlignment = Alignment.CenterVertically) {
            RadioButton(
                selected = StreamEditorVM.RadioSelector.SelectedRadio == SType.OTHER,
                onClick = { StreamEditorVM.RadioSelector.onSelected(SType.OTHER) })
            Text("MP4/MKV/AVI/...")
        }
        Row(verticalAlignment = Alignment.CenterVertically) {
            RadioButton(
                selected = StreamEditorVM.RadioSelector.SelectedRadio == SType.M3U8,
                onClick = { StreamEditorVM.RadioSelector.onSelected(SType.M3U8) })
            Text("M3U8")
        }
    }
    @Composable
        ...
        Button(
            onClick = {
                corScore.launch(Dispatchers.Default) {
                    if(StreamEditorVM.MakeChecks()){
                        StreamEditorVM.EditButton.onPressed()
